...
const DEFAULT_PERMISSIONS = {
  ...
  gifts: false, 'sales-report': false, expenses: false,
  pl: false, cashflow: false, employees: false,
...
export default function UserManagement() {
  ...
  function handleSave() {
    ...
    if (editingId) {
      updated = users.map(u => {
        if (u.id === editingId) {
          const updatedUser = {
            ...u, name: form.name, username: form.username,
            role: form.role,
      ...
      const newUser = {
        id: generateId(), name: form.name, username: form.username,
  ...
  function resetForm() {
    setForm({ name: '', username: '', password: '', confirmPassword: '', role: 'cashier', permissions: { ...DEFAULT_PERMISSIONS } });
    setEditingId(null);
    setShowForm(false);
  }

  function togglePermission(pageId) {
    setForm(f => ({ ...f, permissions: { ...f.permissions, [pageId]: !f.permissions[pageId] } }));
  }
  ...
  const TABS = [
    ...
    { id: 'notifications', label: `${language === 'ar' ? 'الإشعارات' : 'Notifications'} ${unreadCount > 0 ? `(${unreadCount})` : ''}` },
  ];
    ...
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', fontFamily }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
          {activeTab === 'users' && (
            <button onClick={() => { resetForm(); setShowForm(true); }} style={{
              ...
              color: COLORS.white, fontSize: 13, fontWeight: 600, cursor: 'pointer',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={card.label} style={{ background: COLORS.white, borderRadius: 10, border: `1px solid ${COLORS.border}`, padding: '14px 16px', borderTop: `3px solid ${card.color}` }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 800, color: COLORS.charcoal, marginTop: 4 }}>{card.value}</div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {users.map(user => {
                ...
                return (
                  ...
                  >
                    <div style={{ width: 46, height: 46, borderRadius: '50%', flexShrink: 0, background: `linear-gradient(135deg, ${COLORS.charcoal}, ${COLORS.charcoalLight})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: COLORS.white, fontFamily: 'Georgia, serif' }}>
                    ...
                    <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ background: rc.bg, border: `1px solid ${rc.border}`, color: rc.text, fontSize: 10, fontWeight: 600, padding: '1px 8px', borderRadius: 20 }}>{getRoleLabel(user.role)}</span>
                        {isCurrentUser && (
                          <span style={{ background: `${COLORS.red}15`, border: `1px solid ${COLORS.red}44`, color: COLORS.red, fontSize: 10, fontWeight: 600, padding: '1px 8px', borderRadius: 20 }}>
                            {language === 'ar' ? 'أنت' : 'You'}
                          </span>
                        )}
                      </div>
                      <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2 }}>
                        @{user.username} · {user.lastLogin ? `${t('lastLogin')}: ${new Date(user.lastLogin).toLocaleString(language === 'ar' ? 'ar-IQ' : 'en-GB')}` : t('neverLoggedIn')}
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }} onClick={e => e.stopPropagation()}>
                      <button onClick={() => handleEdit(user)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
                      {!isCurrentUser && user.role !== 'superadmin' && (
                        <button onClick={() => setDeleteConfirm(user.id)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
                      )}
                    </div>
                  </div>
                );
            ...
            <div style={{ maxHeight: 500, overflowY: 'auto' }}>
              {accessLog.map((entry, i) => (
                <div key={entry.id} style={{
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: entry.action === 'login' ? `${COLORS.success}20` : `${COLORS.red}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
                    {entry.action === 'login' ? '🟢' : '🔴'}
                  </div>
                  <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.charcoal }}>{entry.name} (@{entry.username})</div>
                  ...
                  <div style={{ padding: '3px 10px', borderRadius: 20, fontSize: 10, fontWeight: 600, background: entry.action === 'login' ? `${COLORS.success}15` : `${COLORS.red}15`, color: entry.action === 'login' ? COLORS.success : COLORS.red, border: `1px solid ${entry.action === 'login' ? COLORS.success : COLORS.red}33` }}>
      ...
      {activeTab === 'notifications' && (
        <div style={{ background: COLORS.white, borderRadius: 12, border: `1px solid ${COLORS.border}`, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
            ...
            {unreadCount > 0 && (
              <span style={{ background: COLORS.red, color: COLORS.white, borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700 }}>{unreadCount}</span>
            )}
          </div>
          {notifications.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: COLORS.textMuted, fontSize: 14 }}>{t('noNotifications')}</div>
          ) : (
            <div style={{ maxHeight: 500, overflowY: 'auto' }}>
              {notifications.map((notif, i) => (
                <div key={notif.id} style={{
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, background: notif.type === 'login' ? `${COLORS.success}20` : `${COLORS.warning}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                    {notif.type === 'login' ? '🔔' : '👋'}
                  </div>
                  <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                    <div style={{ fontSize: 13, fontWeight: notif.read ? 400 : 600, color: COLORS.charcoal }}>{notif.message}</div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2 }}>
                      {new Date(notif.time).toLocaleString(language === 'ar' ? 'ar-IQ' : 'en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
                  {!notif.read && <div style={{ width: 8, height: 8, borderRadius: '50%', background: COLORS.red, flexShrink: 0 }} />}
                </div>
              ))}
            </div>
          )}
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 580, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div>
                ...
                <input value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} placeholder={t('username')} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: COLORS.charcoalMid, marginBottom: 5 }}>{editingId ? t('newPassword') : t('password') + ' *'}</div>
                <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder={editingId ? (language === 'ar' ? 'اتركه فارغاً للإبقاء' : 'Leave blank to keep') : t('password')} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: COLORS.charcoalMid, marginBottom: 5 }}>{t('confirmPassword')}</div>
                <input type="password" value={form.confirmPassword} onChange={e => setForm({ ...form, confirmPassword: e.target.value })} placeholder={t('confirmPassword')} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
              ...
              <div style={{ marginTop: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                    <button onClick={() => setAllPermissions(true)} style={{ padding: '4px 12px', borderRadius: 6, border: `1px solid ${COLORS.success}44`, background: `${COLORS.success}12`, color: COLORS.success, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>{t('allPermissions')}</button>
                    <button onClick={() => setAllPermissions(false)} style={{ padding: '4px 12px', borderRadius: 6, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}12`, color: COLORS.red, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>{t('noPermissions')}</button>
                  </div>
                </div>
                    ...
                    <div key={page.id} onClick={() => togglePermission(page.id)} style={{
                      ...
                      <span style={{ fontSize: 12, fontWeight: 500, color: form.permissions[page.id] ? COLORS.success : COLORS.charcoalMid, flex: 1 }}>
                        {t(page.id === 'purchase-orders' ? 'purchaseOrders' :
                          ...
                          page.id === 'pl' ? 'pl' :
                          page.id === 'gifts' ? 'gifts' : page.id)}
                      </span>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, background: form.permissions[page.id] ? COLORS.success : COLORS.border, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: COLORS.white, fontWeight: 700 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={handleSave} style={{ padding: '9px 24px', borderRadius: 7, border: 'none', background: `linear-gradient(135deg, ${COLORS.red}, ${COLORS.redDark})`, color: COLORS.white, fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>{editingId ? t('save') : t('addUser')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {viewUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 480, boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                <div style={{ fontSize: 18, fontWeight: 700, color: COLORS.charcoal }}>{viewUser.name}</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 4, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                  {(() => { const rc = getRoleColor(viewUser.role); return <span style={{ background: rc.bg, border: `1px solid ${rc.border}`, color: rc.text, fontSize: 11, fontWeight: 600, padding: '2px 10px', borderRadius: 20 }}>{getRoleLabel(viewUser.role)}</span>; })()}
                </div>
              </div>
              <button onClick={() => setViewUser(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: COLORS.textMuted }}>✕</button>
            </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {ALL_PAGES.map(page => {
                    ...
                    return (
                      <div key={page.id} style={{ padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 500, background: hasAccess ? `${COLORS.success}12` : COLORS.offWhite, border: `1px solid ${hasAccess ? COLORS.success + '44' : COLORS.border}`, color: hasAccess ? COLORS.success : COLORS.textMuted, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <span>{page.icon}</span>
                        <span>{t(page.id === 'purchase-orders' ? 'purchaseOrders' : page.id === 'sales-report' ? 'salesReport' : page.id === 'gifts' ? 'gifts' : page.id)}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {viewUser.role === 'superadmin' && (
              <div style={{ background: `${COLORS.red}12`, border: `1px solid ${COLORS.red}33`, borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13, fontWeight: 600, color: COLORS.red, textAlign: 'center' }}>
                {language === 'ar' ? '👑 صلاحية كاملة — مدير النظام الرئيسي' : '👑 Full Access — Super Admin'}
              </div>
            )}

            <div style={{ display: 'flex', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              <button onClick={() => handleEdit(viewUser)} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
              {viewUser.id !== currentUser?.id && viewUser.role !== 'superadmin' && (
                <button onClick={() => { setViewUser(null); setDeleteConfirm(viewUser.id); }} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
              )}
            </div>
          </div>
        </div>

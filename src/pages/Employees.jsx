...
export default function Employees() {
  ...
  function handleSave() {
    ...
    if (!form.salary) return alert('Salary is required');
  ...
  function handleEdit(emp) {
    setForm({
      ...
      notes: emp.notes || '', nationalId: emp.nationalId || ''
    });
    setEditingId(emp.id);
    setShowForm(true);
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Total Employees', value: employees.length, color: COLORS.info },
          { label: 'Active', value: activeCount, color: COLORS.success },
          { label: 'Inactive', value: employees.length - activeCount, color: COLORS.warning },
          { label: 'Monthly Salaries', value: `$${totalSalaries.toFixed(2)}`, color: COLORS.red },
        ].map(card => (
      ...
      {employees.length > 0 && (
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: COLORS.textMuted }}>By Role:</span>
          {ROLES.map(role => {
            ...
            return (
              <div key={role} style={{
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: rc.text }}>{role}</span>
                <span style={{
                  background: rc.text, color: COLORS.white,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="employee@jango.com"
                  style={{
                    width: '100%', padding: '9px 12px',
                    border: `1px solid ${COLORS.border}`, borderRadius: 7,
                    fontSize: 13, color: COLORS.charcoal, outline: 'none',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  {(() => {
                    const rc = ROLE_COLORS[viewEmployee.role] || ROLE_COLORS.Other;
                    return (
                      <span style={{
                        background: rc.bg, border: `1px solid ${rc.border}`,
                  ...
                  <span style={{
                    background: viewEmployee.status === 'Active' ? `${COLORS.success}15` : `${COLORS.warning}15`,
                    border: `1px solid ${viewEmployee.status === 'Active' ? COLORS.success : COLORS.warning}44`,
                    color: viewEmployee.status === 'Active' ? COLORS.success : COLORS.warning,
              ...
              <button onClick={() => setViewEmployee(null)} style={{
            ...
            <div style={{
              ...
              {[
                ...
                { label: 'National ID', value: viewEmployee.nationalId || '—' },
                { label: 'Start Date', value: new Date(viewEmployee.startDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) },
              ].map(row => (
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 12, color: COLORS.textMuted }}>Monthly Salary</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: COLORS.success, fontFamily: 'Georgia, serif' }}>
                  ${viewEmployee.salary.toFixed(2)}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 12, color: COLORS.textMuted }}>Annual</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: COLORS.charcoal }}>
                  ${(viewEmployee.salary * 12).toFixed(2)}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => handleEdit(viewEmployee)} style={{
              ...
              <button onClick={() => setDeleteConfirm(viewEmployee.id)} style={{
                flex: 1, padding: '9px 0', borderRadius: 7,
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{
                      background: rc.bg, border: `1px solid ${rc.border}`,
                      color: rc.text, fontSize: 10, fontWeight: 600,
                      padding: '1px 8px', borderRadius: 20
                    }}>
                      {emp.role}
                    </span>
                    <span style={{
                      background: isActive ? `${COLORS.success}15` : `${COLORS.warning}15`,
                      border: `1px solid ${isActive ? COLORS.success : COLORS.warning}44`,
                ...
                <div style={{ textAlign: 'right', marginRight: 8 }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, color: COLORS.charcoal, fontFamily: 'Georgia, serif' }}>

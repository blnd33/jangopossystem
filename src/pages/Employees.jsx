...
const ROLES_AR = {
  ...
  Warehouse: 'مستودع', Driver: 'سائق', Other: 'أخرى'
};
...
export default function Employees() {
  ...
  function getRoleLabel(role) {
    return language === 'ar' ? (ROLES_AR[role] || role) : role;
  }
  ...
  function handleEdit(emp) {
    setForm({
      ...
      email: emp.email || '', salary: emp.salary, startDate: emp.startDate,
    ...
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', fontFamily }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: COLORS.textMuted, marginTop: 2 }}>
            {employees.length} {t('employees')} · {activeCount} {t('active')}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: t('monthlySalaries'), value: `$${totalSalaries.toFixed(2)}`, color: COLORS.red },
        ].map(card => (
          ...
          {ROLES.map(role => {
            ...
            return (
              <div key={role} style={{
                ...
                <span style={{
                  background: rc.text, color: COLORS.white, borderRadius: '50%',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div>
                ...
                <input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}
                  placeholder="+964 750 000 0000" style={{
                    width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`,
              ...
              <div>
                ...
                <input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                  placeholder="employee@jango.com" style={{
                    width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`,
                    borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box'
                  }} />
              </div>
              ...
              {/* Salary */}
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: COLORS.charcoalMid, marginBottom: 5 }}>{t('salary')} *</div>
              ...
              <div>
                ...
                <input type="date" value={form.startDate} onChange={e => setForm({ ...form, startDate: e.target.value })} style={{
                  width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`,
                  borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box'
                }} />
              </div>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input value={form.nationalId} onChange={e => setForm({ ...form, nationalId: e.target.value })}
                  placeholder={t('nationalId')} style={{
                    width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`,
                    borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none',
                    boxSizing: 'border-box', textAlign: isRTL ? 'right' : 'left'
                  }} />
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              }}>{editingId ? t('save') : t('addEmployee')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {viewEmployee && (
        <div style={{
          ...
          <div style={{
                  ...
                  {(() => {
                    ...
                    return (
                      <span style={{
                        ...
                        {getRoleLabel(viewEmployee.role)}
                      </span>
                    );
                  })()}
                  <span style={{
            ...
            <div style={{
              background: `${COLORS.success}12`, border: `1px solid ${COLORS.success}44`,
              borderRadius: 10, padding: '14px 18px', marginBottom: 16,
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              flexDirection: isRTL ? 'row-reverse' : 'row'
            }}>
              <div style={{ textAlign: isRTL ? 'right' : 'left' }}>
                <div style={{ fontSize: 12, color: COLORS.textMuted }}>{t('salary')}</div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {filtered.map(emp => {
            ...
            return (
              <div key={emp.id} onClick={() => setViewEmployee(emp)} style={{
                background: COLORS.white, borderRadius: 10,
                border: `1px solid ${COLORS.border}`,
                padding: '14px 20px',

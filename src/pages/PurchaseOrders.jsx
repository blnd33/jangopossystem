...
export default function PurchaseOrders() {
  ...
  const [newItem, setNewItem] = useState({ productId: '', productName: '', qty: '', costPrice: '' });
  ...
  function getStatusLabel(status) {
    const labels = { Draft: t('draft'), Ordered: t('ordered'), Received: t('received'), Paid: t('paid') };
  ...
  function handleEdit(order) {
    setForm({ supplierId: order.supplierId, orderDate: order.orderDate, expectedDate: order.expectedDate || '', status: order.status, notes: order.notes || '', items: order.items || [] });
  ...
  function handleStatusChange(id, newStatus) {
    ...
    const updated = orders.map(o => o.id === id ? { ...o, status: newStatus, receivedDate: newStatus === 'Received' ? new Date().toISOString().split('T')[0] : o.receivedDate } : o);
    savePurchaseOrders(updated);
    setOrders(updated);
  ...
  return (
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', fontFamily }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 700, color: COLORS.charcoal, fontFamily: language === 'ar' ? 'Arial, sans-serif' : 'Georgia, serif' }}>{t('purchaseOrders')}</div>
          <div style={{ fontSize: 13, color: COLORS.textMuted, marginTop: 2 }}>
            {orders.length} · <span style={{ color: COLORS.red, fontWeight: 600 }}>{fmt(totalOwed)} {t('owedToSuppliers')}</span>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={card.label} onClick={() => setFilterStatus(card.label === t('draft') ? 'Draft' : card.label === t('ordered') ? 'Ordered' : card.label === t('received') ? 'Received' : 'Paid')}
            style={{ background: COLORS.white, borderRadius: 10, border: `1px solid ${COLORS.border}`, padding: '14px 16px', cursor: 'pointer', borderTop: `3px solid ${card.color}` }}>
            <div style={{ fontSize: 11, color: COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.8 }}>{card.label}</div>
            <div style={{ fontSize: 28, fontWeight: 800, color: card.color, marginTop: 4, fontFamily: language === 'ar' ? 'Arial, sans-serif' : 'Georgia, serif' }}>{card.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
        ...
        <select value={filterSupplier} onChange={e => setFilterSupplier(e.target.value)} style={{ padding: '10px 14px', border: `1px solid ${COLORS.border}`, borderRadius: 8, fontSize: 13, color: COLORS.charcoal, outline: 'none', background: COLORS.white, cursor: 'pointer' }}>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 620, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <select value={form.supplierId} onChange={e => setForm({ ...form, supplierId: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', background: COLORS.white, boxSizing: 'border-box' }}>
              ...
              <div>
                ...
                <input type="date" value={form.orderDate} onChange={e => setForm({ ...form, orderDate: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: COLORS.charcoalMid, marginBottom: 5 }}>{t('expectedDate')}</div>
                <input type="date" value={form.expectedDate} onChange={e => setForm({ ...form, expectedDate: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder={t('notes')} rows={2} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', resize: 'vertical', boxSizing: 'border-box', textAlign: isRTL ? 'right' : 'left' }} />
              </div>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 16, marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 8, marginBottom: 8 }}>
                ...
                }} style={{ padding: '8px 10px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 12, color: COLORS.charcoal, outline: 'none', background: COLORS.white }}>
                  <option value="">{t('selectSupplier')}</option>
                  {supplierProducts.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
                <input type="number" placeholder={t('stockQty')} value={newItem.qty} onChange={e => setNewItem({ ...newItem, qty: e.target.value })} style={{ padding: '8px 10px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 12, outline: 'none' }} />
                <input type="number" placeholder={t('costPrice')} value={newItem.costPrice} onChange={e => setNewItem({ ...newItem, costPrice: e.target.value })} style={{ padding: '8px 10px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 12, outline: 'none' }} />
                <button onClick={addItem} style={{ padding: '8px 14px', borderRadius: 7, border: 'none', background: COLORS.charcoal, color: COLORS.white, fontSize: 12, cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap' }}>+ {t('add')}</button>
              </div>
              <input placeholder={t('manualName')} value={newItem.productName} onChange={e => setNewItem({ ...newItem, productName: e.target.value, productId: '' })} style={{ width: '100%', padding: '8px 12px', marginBottom: 12, border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 12, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box', textAlign: isRTL ? 'right' : 'left' }} />

              {form.items.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '20px 0', color: COLORS.textMuted, fontSize: 13, border: `1px dashed ${COLORS.border}`, borderRadius: 8 }}>{t('noData')}</div>
              ) : (
                    ...
                    <tbody>
                      {form.items.map((item, i) => (
                        <tr key={item.id} style={{ borderTop: `1px solid ${COLORS.border}`, background: i % 2 === 0 ? COLORS.white : `${COLORS.offWhite}66` }}>
                          ...
                          <td style={{ padding: '8px 12px', textAlign: 'right' }}>{fmt(item.costPrice)}</td>
                          <td style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 600 }}>{fmt(item.qty * item.costPrice)}</td>
                      ...
                      <tr style={{ borderTop: `2px solid ${COLORS.border}`, background: COLORS.offWhite }}>
                        ...
                        <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 800, color: COLORS.charcoal, fontSize: 15 }}>
                          {fmt(form.items.reduce((sum, i) => sum + i.qty * i.costPrice, 0))}
                        </td>
                        <td />
                      </tr>
                    </tbody>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={handleSave} style={{ padding: '9px 24px', borderRadius: 7, border: 'none', background: `linear-gradient(135deg, ${COLORS.red}, ${COLORS.redDark})`, color: COLORS.white, fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>{editingId ? t('save') : t('newPO')}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 540, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={() => setViewOrder(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: COLORS.textMuted }}>✕</button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                {STATUSES.map(s => {
                  ...
                  return (
                    <button key={s} onClick={() => handleStatusChange(viewOrder.id, s)} style={{ padding: '6px 14px', borderRadius: 6, cursor: 'pointer', border: `1px solid ${isActive ? sc.text : COLORS.border}`, background: isActive ? sc.bg : COLORS.white, color: isActive ? sc.text : COLORS.charcoalMid, fontSize: 12, fontWeight: isActive ? 600 : 400 }}>
                      {getStatusLabel(s)}
                    </button>
                  );
                })}
              ...
              {viewOrder.status === 'Received' && (
                <div style={{ marginTop: 8, fontSize: 12, color: COLORS.success, fontWeight: 600, background: `${COLORS.success}12`, padding: '6px 12px', borderRadius: 6, border: `1px solid ${COLORS.success}33` }}>
                  ✅ {t('stockUpdated')}
                </div>
              )}
            </div>
              ...
              <div style={{ border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>
                    ...
                    <tr style={{ borderTop: `2px solid ${COLORS.border}`, background: COLORS.offWhite }}>
                      <td colSpan={3} style={{ padding: '10px 12px', fontWeight: 700, color: COLORS.charcoal, textAlign: isRTL ? 'right' : 'left' }}>{t('total')}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 800, color: COLORS.charcoal, fontSize: 16 }}>{fmt(viewOrder.total)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {viewOrder.notes && (
              <div style={{ background: COLORS.offWhite, borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: COLORS.charcoalMid, fontStyle: 'italic', textAlign: isRTL ? 'right' : 'left' }}>{viewOrder.notes}</div>
            )}

            <div style={{ display: 'flex', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              <button onClick={() => handleEdit(viewOrder)} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
              <button onClick={() => setDeleteConfirm(viewOrder.id)} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {filtered.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(order => {
            ...
            return (
              <div key={order.id} onClick={() => setViewOrder(order)} style={{ background: COLORS.white, borderRadius: 10, border: `1px solid ${COLORS.border}`, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14, flexDirection: isRTL ? 'row-reverse' : 'row', cursor: 'pointer', transition: 'all 0.15s', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = COLORS.red + '66'}
                onMouseLeave={e => e.currentTarget.style.borderColor = COLORS.border}
              >
                <div style={{ width: 52, height: 52, borderRadius: 10, flexShrink: 0, background: `${COLORS.charcoal}12`, border: `1px solid ${COLORS.charcoal}22`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                    ...
                    <span style={{ background: sc.bg, border: `1px solid ${sc.border}`, color: sc.text, fontSize: 10, fontWeight: 600, padding: '1px 8px', borderRadius: 20 }}>{getStatusLabel(order.status)}</span>
                  </div>
                  <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2 }}>
                    {order.items?.length || 0} {t('items')} · {t('orderDate')}: {new Date(order.orderDate).toLocaleDateString(language === 'ar' ? 'ar-IQ' : 'en-GB', { day: 'numeric', month: 'short' })}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }} onClick={e => e.stopPropagation()}>
                  <button onClick={() => handleEdit(order)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
                  <button onClick={() => setDeleteConfirm(order.id)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
                </div>
              </div>
            );
          })}

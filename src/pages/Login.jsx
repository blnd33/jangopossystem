      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: 28, fontWeight: 900, color: COLORS.charcoal, fontFamily: 'Georgia, serif' }}>J</span>
          </div>
          ...
          <div style={{ width: 50, height: 3, background: `linear-gradient(90deg, transparent, ${C.red}, transparent)`, margin: '8px auto' }} />
        ...
        <div style={{ padding: '32px 40px 36px', background: C.white }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: C.charcoal, marginBottom: 24, textAlign: 'center', fontFamily: language === 'ar' ? 'Arial, sans-serif' : 'Georgia, serif' }}>
            {t('welcomeBack')} 👋
          </div>
          ...
          <form onSubmit={handleLogin}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{
                    ...
                    paddingLeft: isRTL ? 16 : 44, paddingRight: isRTL ? 44 : 16,
                ...
                />
                <span style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', left: isRTL ? 'auto' : 14, right: isRTL ? 14 : 'auto', fontSize: 16, opacity: 0.4 }}>👤</span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{
                    width: '100%', padding: '12px 16px',
                    paddingLeft: isRTL ? 16 : 44, paddingRight: isRTL ? 44 : 44,
                    border: `1.5px solid ${error ? C.red : C.border}`,
                    borderRadius: 9, fontSize: 14, color: C.charcoal,
                    outline: 'none', boxSizing: 'border-box',
                    background: C.white,
                ...
                />
                <span style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', left: isRTL ? 'auto' : 14, right: isRTL ? 14 : 'auto', fontSize: 16, opacity: 0.4 }}>🔒</span>
                <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', top: '50%', transform: 'translateY(-50%)', right: isRTL ? 'auto' : 12, left: isRTL ? 12 : 'auto', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, opacity: 0.5, padding: 4 }}>
                  {showPassword ? '🙈' : '👁️'}
                </button>
              </div>
            </div>
            ...
            {/* Error */}
            {error && (
              <div style={{ background: `${C.red}12`, border: `1px solid ${C.red}44`, borderRadius: 8, padding: '10px 14px', fontSize: 13, color: C.red, fontWeight: 500, marginBottom: 16, textAlign: isRTL ? 'right' : 'left' }}>
                ❌ {error}
              </div>
            )}
        ...
        {/* Footer */}
        <div style={{ padding: '12px 40px 20px', textAlign: 'center', borderTop: `1px solid ${C.border}`, background: C.white }}>
          <div style={{ fontSize: 11, color: C.textMuted }}>{t('version')} · Jango Furniture</div>
        </div>
      </div>
    </div>
  );

...
import { getReturns, saveReturns, getCustomers, getSales, getProducts, saveProducts, generateId } from '../data/store';
import { useLanguage } from '../data/LanguageContext';
import { useCurrency } from '../contexts/CurrencyContext';

const REASONS = ['Damaged', 'Wrong Item', 'Changed Mind', 'Defective', 'Other'];
const REASONS_AR = { Damaged: 'تالف', 'Wrong Item': 'منتج خاطئ', 'Changed Mind': 'تغيير الرأي', Defective: 'معيب', Other: 'أخرى' };
...
export default function Returns() {
  ...
  function getReasonLabel(reason) { return language === 'ar' ? (REASONS_AR[reason] || reason) : reason; }
  function getTypeLabel(type) { return language === 'ar' ? (type === 'Refund' ? t('refund') : t('exchange')) : type; }
  ...
  function handleEdit(ret) {
    setForm({ customerId: ret.customerId || '', customerName: ret.customerName, saleId: ret.saleId || '', returnType: ret.returnType, reason: ret.reason, status: ret.status, items: ret.items, refundAmount: ret.refundAmount || '', notes: ret.notes || '', date: ret.date, restockItems: ret.restockItems !== false });
    setEditingId(ret.id);
    setShowForm(true);
    setViewReturn(null);
  }
  ...
  function handleStatusChange(id, newStatus) {
    ...
    if (newStatus === 'Approved' && ret.status !== 'Approved' && ret.restockItems) {
      const updatedProducts = products.map(p => {
        if (ret.items.toLowerCase().includes(p.name.toLowerCase())) return { ...p, stock: p.stock + 1 };
        return p;
      });
  ...
  function resetForm() {
    setForm({ customerId: '', customerName: '', saleId: '', returnType: 'Refund', reason: 'Damaged', status: 'Pending', items: '', refundAmount: '', notes: '', date: new Date().toISOString().split('T')[0], restockItems: true });
    setEditingId(null);
    setShowForm(false);
  }

  const filtered = returns.filter(r => {
    const matchSearch = r.customerName.toLowerCase().includes(search.toLowerCase()) || r.items.toLowerCase().includes(search.toLowerCase());
  ...
  const customerSales = sales.filter(s => form.customerId ? s.customerId === form.customerId : true);
  ...
  return (
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', fontFamily }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 700, color: COLORS.charcoal, fontFamily: language === 'ar' ? 'Arial, sans-serif' : 'Georgia, serif' }}>{t('returns')}</div>
          <div style={{ fontSize: 13, color: COLORS.textMuted, marginTop: 2 }}>
            {returns.length} · {pendingCount} {t('pending')} ·
            <span style={{ color: COLORS.red, fontWeight: 600 }}> {fmt(totalRefunded)} {t('totalRefunded')}</span>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: t('totalRefunded'), value: fmt(totalRefunded), color: COLORS.info },
        ].map(card => (
          ...
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 540, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })} placeholder={t('manualName')} style={{ width: '100%', padding: '9px 12px', marginTop: 6, border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box', textAlign: isRTL ? 'right' : 'left' }} />
              </div>
              ...
              {customerSales.length > 0 && (
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <select value={form.saleId} onChange={e => setForm({ ...form, saleId: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', background: COLORS.white, boxSizing: 'border-box' }}>
                    <option value="">{t('noLinkedSale')}</option>
                    {customerSales.map(s => <option key={s.id} value={s.id}>#{s.id.slice(-6).toUpperCase()} — {fmt(s.total)}</option>)}
                  </select>
                </div>
              )}
              ...
              <div>
                ...
                <select value={form.returnType} onChange={e => setForm({ ...form, returnType: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', background: COLORS.white, boxSizing: 'border-box' }}>
                  {RETURN_TYPES.map(type => <option key={type} value={type}>{getTypeLabel(type)}</option>)}
                </select>
              </div>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input value={form.items} onChange={e => setForm({ ...form, items: e.target.value })} placeholder={t('returnedItems')} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box', textAlign: isRTL ? 'right' : 'left' }} />
              </div>

              {form.returnType === 'Refund' && (
                <div style={{ gridColumn: '1 / -1' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: COLORS.charcoalMid, marginBottom: 5 }}>{t('refundAmount')} * (USD)</div>
                  <input type="number" value={form.refundAmount} onChange={e => setForm({ ...form, refundAmount: e.target.value })} placeholder="0.00" style={{ width: '100%', padding: '9px 12px', border: `1px solid ${COLORS.border}`, borderRadius: 7, fontSize: 13, color: COLORS.charcoal, outline: 'none', boxSizing: 'border-box' }} />
                  {form.refundAmount && (
                    <div style={{ fontSize: 12, color: COLORS.info, marginTop: 4, fontWeight: 600 }}>
                      = {fmt(parseFloat(form.refundAmount) || 0)}
                    </div>
                  )}
                </div>
              )}
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                <input type="checkbox" id="restock" checked={form.restockItems} onChange={e => setForm({ ...form, restockItems: e.target.checked })} style={{ width: 16, height: 16, cursor: 'pointer' }} />
                <label htmlFor="restock" style={{ fontSize: 13, color: COLORS.charcoal, cursor: 'pointer' }}>{t('restock')}</label>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={handleSave} style={{ padding: '9px 24px', borderRadius: 7, border: 'none', background: `linear-gradient(135deg, ${COLORS.red}, ${COLORS.redDark})`, color: COLORS.white, fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>{editingId ? t('save') : t('newReturn')}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: COLORS.white, borderRadius: 14, padding: 28, width: 460, boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={() => setViewReturn(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: COLORS.textMuted }}>✕</button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                {STATUSES.map(s => {
                  ...
                  return (
                    <button key={s} onClick={() => handleStatusChange(viewReturn.id, s)} style={{ padding: '6px 16px', borderRadius: 6, cursor: 'pointer', border: `1px solid ${isActive ? sc.text : COLORS.border}`, background: isActive ? sc.bg : COLORS.white, color: isActive ? sc.text : COLORS.charcoalMid, fontSize: 12, fontWeight: isActive ? 600 : 400 }}>
                      {getStatusLabel(s)}
                    </button>
                  );
                })}
            ...
            <div style={{ background: COLORS.offWhite, borderRadius: 10, padding: 16, marginBottom: 16 }}>
              {[
                ...
                viewReturn.returnType === 'Refund' && { label: t('refundAmount'), value: fmt(viewReturn.refundAmount) },
            ...
            <div style={{ display: 'flex', gap: 10, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              <button onClick={() => handleEdit(viewReturn)} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
              <button onClick={() => { setViewReturn(null); setDeleteConfirm(viewReturn.id); }} style={{ flex: 1, padding: '9px 0', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {filtered.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(ret => {
            ...
            return (
              <div key={ret.id} onClick={() => setViewReturn(ret)} style={{ background: COLORS.white, borderRadius: 10, border: `1px solid ${COLORS.border}`, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14, flexDirection: isRTL ? 'row-reverse' : 'row', cursor: 'pointer', transition: 'all 0.15s', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = COLORS.red + '66'}
                onMouseLeave={e => e.currentTarget.style.borderColor = COLORS.border}
              >
                <div style={{ width: 44, height: 44, borderRadius: 10, flexShrink: 0, background: sc.bg, border: `1px solid ${sc.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                ...
                <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ background: sc.bg, border: `1px solid ${sc.border}`, color: sc.text, fontSize: 10, fontWeight: 600, padding: '1px 8px', borderRadius: 20 }}>{getStatusLabel(ret.status)}</span>
                    <span style={{ background: ret.returnType === 'Refund' ? `${COLORS.red}12` : `${COLORS.info}12`, border: `1px solid ${ret.returnType === 'Refund' ? COLORS.red : COLORS.info}33`, color: ret.returnType === 'Refund' ? COLORS.red : COLORS.info, fontSize: 10, fontWeight: 600, padding: '1px 8px', borderRadius: 20 }}>
                      {getTypeLabel(ret.returnType)}
                    </span>
                  </div>
                  <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }} onClick={e => e.stopPropagation()}>
                  <button onClick={() => handleEdit(ret)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.border}`, background: COLORS.white, color: COLORS.charcoalMid, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
                  <button onClick={() => setDeleteConfirm(ret.id)} style={{ padding: '7px 14px', borderRadius: 7, border: `1px solid ${COLORS.red}44`, background: `${COLORS.red}11`, color: COLORS.red, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
                </div>
              </div>
            );
          })}

  ...
  return (
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', fontFamily }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: t('moneyIn'), value: `$${totalInflow.toFixed(2)}`, color: COLORS.success, sub: t('totalInflow') },
          { label: t('moneyOut'), value: `$${totalOutflow.toFixed(2)}`, color: COLORS.red, sub: t('totalOutflow') },
          { label: t('netFlow'), value: `$${netFlow.toFixed(2)}`, color: netFlow >= 0 ? COLORS.success : COLORS.red, sub: netFlow >= 0 ? t('positive') : t('negative') },
          { label: t('endingBalance'), value: `$${endingBalance.toFixed(2)}`, color: COLORS.info, sub: t('runningTotal') },
        ].map(card => (
                ...
                <thead>
                  <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
                    {[t('period'), t('in'), t('out'), t('net')].map(h => (
                      <th key={h} style={{
                        padding: '6px 8px',
                        textAlign: h === t('period') ? (isRTL ? 'right' : 'left') : 'right',
                ...
                <tbody>
                  {flowData.filter(d => d.inflow > 0 || d.outflow > 0).map((row, i) => (
                    <tr key={i} style={{
                      ...
                      <td style={{ padding: '7px 8px', textAlign: 'right', color: COLORS.success, fontWeight: 500 }}>${row.inflow.toFixed(2)}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'right', color: COLORS.red, fontWeight: 500 }}>${row.outflow.toFixed(2)}</td>

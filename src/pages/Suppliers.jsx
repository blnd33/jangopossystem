        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: C.white, borderRadius: 14, padding: 28, width: 500, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.25)', direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 5 }}>
                  {t('email')} <span style={{ fontSize: 10, color: C.textMuted, fontWeight: 400 }}>({opt})</span>
                </div>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 5 }}>
                  {t('address')} <span style={{ fontSize: 10, color: C.textMuted, fontWeight: 400 }}>({opt})</span>
                </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              ...
              <button onClick={handleSave} style={{ padding: '9px 24px', borderRadius: 7, border: 'none', background: `linear-gradient(135deg, ${C.red}, ${C.redDark})`, color: '#fff', fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>{editingId ? t('save') : t('addSupplier')}</button>
            </div>
          </div>
        </div>
      )}
          ...
          {filtered.map(supplier => (
            <div key={supplier.id} style={{
              ...
              <div style={{
                ...
                fontSize: 18, fontWeight: 700, color: C.red, fontFamily: 'Georgia, serif'
              }}>
              ...
              <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
                ...
                {supplier.notes && (
                  <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2, fontStyle: 'italic' }}>{supplier.notes}</div>
                )}
              </div>

              {/* Actions */}
              <div style={{ display: 'flex', gap: 8, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                <button onClick={() => handleEdit(supplier)} style={{ padding: '7px 16px', borderRadius: 7, border: `1px solid ${C.border}`, background: C.white, color: C.charcoalMid, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('edit')}</button>
                <button onClick={() => setDeleteConfirm(supplier.id)} style={{ padding: '7px 16px', borderRadius: 7, border: `1px solid ${C.red}44`, background: `${C.red}11`, color: C.red, fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('delete')}</button>
              </div>
            </div>
          ))}
        </div>
